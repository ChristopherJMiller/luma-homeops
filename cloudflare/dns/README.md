...
```
chrismiller.xyz    A      <WAN IP>          proxied   ┐ content kept current by
realliance.net     A      <WAN IP>          DNS-only  ┘ cloudflareddns (WAN is DHCP)
*.chrismiller.xyz  CNAME  chrismiller.xyz   DNS-only  ┐ static — every hostname
*.realliance.net   CNAME  realliance.net    DNS-only  ┘ reaches Traefik, which
...
- `buttert.art`, `werethemille.rs` — apex only, cloudflareddns-maintained.
