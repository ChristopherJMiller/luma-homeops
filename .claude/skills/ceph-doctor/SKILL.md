...
0. **Is it actually a data problem?** Before anything else, separate *advisory* health from *data-path* health. `HEALTH_ERR` with `113 pgs active+clean, 6 osds up/in` is not an outage.
   ...
   | `RECENT_CRASH` | Listed until archived. **Read them before archiving** — `ceph crash ls-new` then `ceph crash info <id> \| jq -r .assert_msg`. The 2026-09-11 batch turned out to be `mon.t` losing its RocksDB store (`MonitorDBStore … failed to write to db`) and being silently failed over (#2661). `ceph crash archive-all` once understood. | Read first |
